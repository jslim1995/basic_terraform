...
resource "aws_vpc" "main" {
  ...
  tags = {
    ...
    # test = "${module.vault.prefix_vault}-test"
  }
...
resource "aws_subnet" "sb" {
  ...
  tags = {
    Name = "${var.prefix}-subnet-public${tonumber(count.index) + 1}-${var.subnet_az_list[count.index].availability_zone}"
  }
}
...
#   count         = 1
  ...
  key_name        = "${var.prefix}-ssh-key.pem"
  tags = {
