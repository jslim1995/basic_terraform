#!/bin/bash
echo -e "Script started.\nPlease wait for a few moments before the \"Script Completed\" message is displayed.\n"
cd
...
sudo go build -o vault-plugin-database-oracle ./plugin >> $log_file && ls | grep vault-plugin-database-oracle
